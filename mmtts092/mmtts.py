...
class Phenomen:
	...
	def __init__(self,dic_name):
		...
		self.userdic = 'userdic.txt'	# 辞書の場所
		...
		self.flag_reading_symbol = 0	# 記号を読むかどうかのフラグ

		self.use_accent = 1				# アクセントを処理するかのフラグ
		...
		if self.mecab_dic_name == 'ipadic':
			...
			raise Exception( self.mecab_dic_name + 'は準備されていない辞書です。')

		# MeCab辞書の存在を確認する
		if not os.path.isdir(mecab_dic_dir):
			raise Exception('指定のMeCab辞書' + self.mecab_dic_name + 'が見つかりません。')
	...
	def convert(self,s):
		...
		if DEBUG:
			## log
			self.log.writeln('仮名文字から音素配列に変換後:')
			for n in phonemes:
				self.log.write(n)
			self.log.writeln()
			self.log.writeln()
	...
	def make_pho_lines( self, nodes ):
		...
		for node in nodes:
			...
			if DEBUG:
				...
				for n in node[0]:
					for m in n:
						self.log.write(m)
				self.log.writeln()
				self.log.writeln("品詞　　　：" + node[1])
				if node[2]:
					self.log.writeln("アクセント：" + node[2])
				if node[1] !='補助記号' and node[1] != '句点':
					self.log.writeln("モーラ数　：" + str(len(node[0])))
				self.log.writeln()
				# 音素の情報　　（記号、長さ、高低）
			...
			if ( node[1] == '助詞' ) and prev_noun and prev_accent == '0':
		...
		if DEBUG:
			self.log.writeln('疑問形処理後:')
			self.log.dump_phonemes( phonemes )


		# 「です」の擬似無声母音化（後日汎用化する予定）
		...
		while node:
			...
			if DEBUG:
				...
				if nodes:
					...
					for n in nodes:
						self.log.writeln("品詞：" + n[1])

						if n[2]:
							accent = n[2]
							self.log.writeln("アクセント：" + accent)
						else:
							self.log.writeln("アクセント：情報なし")
						self.log.write("音素記号：")
						for l in n[0]:
							self.log.write('[')
							for m in l:
...
def main():
	...
	if options.enable_stdin != None:
		...
		if options.filename:
			print "標準入力とファイルを同時に入力にすることはできません"
		else:
