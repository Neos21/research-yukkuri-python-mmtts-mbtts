...
if os.path.exists( config_path ):
	...
	if openjtalk_charset  == 'SHIFT_JIS':
		openjtalk_dic_name = 'open_jtalk_dic_shift_jis-1.00'
	elif openjtalk_charset  == 'EUC-JP':
		openjtalk_dic_name = 'open_jtalk_dic_euc_jp-1.00'
	else:
		openjtalk_dic_name = 'open_jtalk_dic_utf_8-1.00'
	...
	for f in os.listdir('.'):
		if re.match('hts_voice_.*\.tar\.gz$',f):
			try:
				...
				print f + u' のコピーに失敗しました。'

	# openjtalkの辞書のパス
	if openjtalk_dic_name:
		f = os.path.join( openjtalk_dic_name + '.tar.gz' )
		if os.path.exists(f):
			try:
				shutil.copyfile(f,os.path.join(dist,f))
				print f + u' をコピーしました。'
