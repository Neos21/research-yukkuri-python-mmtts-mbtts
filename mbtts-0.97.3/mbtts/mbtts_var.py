...
AQUESTALK2_NAME = 'libAquesTalk2.so.1.0'
...
# 標準の音声名
DEFAULT_VOICE = u'MBROLA.' + mb_jp2.JP2.name
...
# 声種ファイルのディレクトリ
if os.path.exists( os.path.join( CONFIG_DIR, PHONT_DIR_NAME ) ):
	PHONT_DIR = os.path.join( CONFIG_DIR, PHONT_DIR_NAME )
else:
	PHONT_DIR = PHONT_DIR_NAME
...
# AQUESTALK2 DLLファイルが置いてあるパス
# 無ければ None
if os.path.exists( os.path.join( CONFIG_DIR, AQUESTALK2_NAME ) ):
	AQUESTALK2_PATH = os.path.join( CONFIG_DIR, AQUESTALK2_NAME )
else:
	try:
...
# 使用できる声種の一覧
PHONTS = []
if os.path.exists(PHONT_DIR):
	for _i in os.listdir(PHONT_DIR):
		m = re.match( '(.+)\.phont$', _i )
		if m:
