...
class MyApp(wx.PySimpleApp):
	...
	def OnInit(self):
		# TTS生成
		self.guide = mbtts.create('MBROLA.jp3')
		voice = 'MBROLA.jp1'
		...
		self.Menu1.Append(self.ID101, u'読み上げる\tCtrl-E')
		self.Menu1.Append(self.ID102, u'停止\tCtrl-S')
		self.Menu1.Append(self.ID103, u'音声ファイルの作成\tCtrl-W')
		self.Menu1.Append(self.ID104, u'テキストボックスのクリア\tCtrl-D')
		self.Menu1.Append(self.ID105, u'終了(&X)')
		self.Menu1.Enable(self.ID102, False )

		for i in mbtts.available_databases():
			t = wx.NewId()
			self.Menu2.AppendRadioItem(t,i)
			self.Frm.Bind(wx.EVT_MENU, self.SelectVoice, id=t)
			if voice == i:
				self.Menu2.Check(t,True)
				self.tts = mbtts.create(i)

		self.MenuBar = wx.MenuBar()
		self.MenuBar.Append(self.Menu1, u'操作(&O)')
		self.MenuBar.Append(self.Menu2, u'音声選択(&S)')
	...
	def SelectVoice(self,event):
		for i in self.Menu2.GetMenuItems():
			if i.IsChecked():
				self.tts = mbtts.create(i.GetLabel())


	def StartCallBack(self):
		self.Menu1.SetLabel(self.ID101, u'一時停止\tCtrl-P')
		self.Frm.Bind(wx.EVT_MENU, self.Pause, id=self.ID101)
		self.Menu1.Enable(self.ID102, True )
		return


	def FinishCallBack(self):
		self.Menu1.Enable(self.ID102, False )
		self.Menu1.SetLabel(self.ID101, u'読み上げる\tCtrl-E')
		self.Frm.Bind(wx.EVT_MENU, self.Say,  id=self.ID101)
		return
	...
	def Pause(self,event):
		self.Menu1.SetLabel(self.ID101, u'再開\tCtrl-P')
