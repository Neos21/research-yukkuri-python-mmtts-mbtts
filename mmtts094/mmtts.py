...
class Phenomen:
	...
	def make_pho_lines( self, nodes ):
		...
		scale = 1 / self.pho_rate / self.default_scale
		...
		for ph in phonemes:
			...
			# 標準値を計算しておく
			pho_pitch_temp = str(int((pt)*self.pho_pitch))
			...
			if data == '.' or data == '!' or data == '?':
				result.append( '_' + '\t' + str(int(tm*scale)) + '\t' + str(int(po)) + '\t' + pho_pitch_temp )
				result.append( ';kuten' )
			...
			# 通常の音素
				...
				# アクセントのある音節の上昇
					...
					pho_pitch_temp = str(int((pt+self.acc_diff)*self.pho_pitch))
				...
				# 疑問文の上昇
					...
					pho_pitch_temp = str(int((pt+self.ques_diff)*self.pho_pitch))

				else:
					pho_pitch_temp = str(int((pt)*self.pho_pitch))

				result.append( data + '\t' + str(int(tm*scale)) + '\t' + str(int(po)) + '\t' + pho_pitch_temp )
...
def makePHO(string,filename):
	...
	# 音素クラス生成
	phenomen = Phenomen( dic_name )							# mecab辞書の指定
...
def makeWAV(string,filename):
	...
	if rate == 0.0:
		rate = 0.1
	rate = 1.0/rate
	os.system( "mbrola -e -v " + str(volume) + " -t" + str(rate) + " -f" + str(pitch) +  " " + database + " " + phofile + " " + filename )
...
def set_volume(vol):
	...
	global volume
	if vol <= 0:
		sys.stderr.write( u"音量比率に０もしくは負の数を指定することはできません\n".encode( system_charset ) )
		sys.exit()

	if vol >= 1.0:
		sys.stderr.write( u"音量比率を１より大きくすることはできません\n".encode( system_charset ) )
		sys.exit()
...
def set_rate(val):
	...
	global rate
	if val <= 0:
		sys.stderr.write( u"読み上げ速度比率に０もしくは負の数を指定することはできません\n".encode( system_charset ) )
		sys.exit()
...
def set_pitch(val):
	...
	global pitch
	if val <= 0:
		sys.stderr.write( u"ピッチ比率に０もしくは負の数を指定することはできません\n".encode( system_charset ) )
		sys.exit()
...
def main():
	...
	parser.add_option( "-r", "--rate",         dest="rate",
		help=u"読み上げ速度の比率を指定します  （0.0＜rate）" )
	parser.add_option( "-v", "--volume",       dest="volume",
		help=u"読み上げ音量の比率を指定します  （0.0＜volume≦1.0）" )
	parser.add_option( "-p", "--pitch",        dest="pitch",
		help=u"読み上げピッチの比率を指定します（0.0＜pitch）" )
